...
public class LessC {
    ...
    public Args parseArguments(String[] args) {
        ...
        ArgumentParser parser = ArgumentParsers.newArgumentParser(PROGRAM_NAME)
                ...
                .setDefault("batch_preserve_path", CompileBatch.DEFAULT_PRESERVE_PATH)
                .setDefault("batch_include_pattern", CompileBatch.DEFAULT_INCLUDE_PATTTERN);

        parser.addArgument("--batch", "-b")
                .action(Arguments.storeTrue())
                .help("Batch mode");

        parser.addArgument("--batch-preserve-path", "-p")
                .action(Arguments.storeTrue())
                .help("Preserve directory structure in batch mode");

        parser.addArgument("--batch-include-pattern", "-a")
                .type(String.class)
                .help("Syntax and Pattern that will be included, default to 'glob:[!_]*.less'. See FileSystem.html#getPathMatcher(java.lang.String) docs");
        ...
        try {
            ...
            cmdArgs.batchPreservePath = res.getBoolean("batch_preserve_path");
            cmdArgs.includePattern = res.getString("batch_include_pattern");
    ...
    public static class Args {
        ...
        private boolean batchPreservePath;
        ...
        public boolean batchPreservePath() {
            return batchPreservePath;
        }
